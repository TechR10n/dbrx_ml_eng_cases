...
# MAGIC - Install cluster libraries using PyPI
# MAGIC - Explore a different approach for allocation modeling
# MAGIC - Calculate performance metrics for multiple models in parallel

# COMMAND ----------

# MAGIC %md
# MAGIC ## Install Prophet
# MAGIC In the last notebook, we used `%pip` to install `fbprophet` to all nodes in the cluster scoped at the _notebook_ level. As we'll continue to use Prophet in this and the next notebook, we can save a few minutes by installing Prophet as a [cluster library](https://docs.databricks.com/libraries/cluster-libraries.html) using [PyPI](https://docs.databricks.com/libraries/workspace-libraries.html#pypi-libraries). Note that this will change the execution environment for all notebooks attached to the cluster and the library will be re-installed when the cluster is restarted
...
# MAGIC
# MAGIC ### Code Challenge: Parallelize the Following Code
# MAGIC
...
# MAGIC
# MAGIC Using the previous notebook and the code below as a guide, see if you can successfully refactor the code in the following 2 cells to parallelize execution.
...
def get_forecast_agg_item(item_pd, days_to_forecast):
...
# MAGIC
# MAGIC In the cell below, join your `item_store_ratios` with the predictions you made above. Save out a Delta Lake table `agg_item_preds` with the following schema:
# MAGIC
# MAGIC | field | type |
# MAGIC | --- | --- |
# MAGIC | store | INT |
...
tablePath = userhome + "/demand/agg_item_preds"
