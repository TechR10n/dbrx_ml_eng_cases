...
def def_discriminator():
    ...
    model.compile(loss=tf.keras.losses.binary_crossentropy, 
                  optimizer=tf.keras.optimizers.legacy.Adam(learning_rate=LR/2, decay=LR/NUM_EPOCH)) # Half the generator's learning rate to help stabilize equilibrium
...
def def_gan(noise_dim, discriminator, generator):
    ...
    gan_model.compile(optimizer=tf.keras.optimizers.legacy.Adam(learning_rate=LR, decay=LR/NUM_EPOCH), 
